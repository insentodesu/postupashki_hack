...
export function DashboardShell({ snapshot, view, origin, connected, warning, botUsername }: { snapshot: DashboardSnapshot; view: string; origin: string; connected: boolean; warning: string | null; botUsername: string }) {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">К содержанию</a>
    <aside className="sidebar">
      ...
      <nav aria-label="Основная навигация">{nav.map(item => <Link key={item.id} href={`/?view=${item.id}&origin=${origin}`} aria-current={view === item.id ? "page" : undefined}>{item.label}</Link>)}</nav>
...
function Overview({ snapshot, totalRevenue, attributedRevenue, unknown, romi }: { snapshot: DashboardSnapshot; totalRevenue: number; attributedRevenue: number; unknown: number; romi: ReturnType<typeof buildRomiRows> }) {
  ...
  return <div className="view-stack">
    ...
    <section className="ledger-card flow-card"><div className="section-heading"><div><h2>Доказательная цепочка</h2></div><span className="section-note">от публикации до оплаты</span></div><div className="flow-rail">
      {[['Placement',snapshot.placements.length,'метки созданы'],['Touch',snapshot.touches.length,'переходы связаны'],['Lead',snapshot.leads.length,'интерес зафиксирован'],['Revenue',rub.format(totalRevenue),'оплаты подтверждены']].map(([label,value,note], index) => <div className="flow-step" key={String(label)}><b>{label}</b><strong>{value}</strong><small>{note}</small>{index < 3 && <i aria-hidden="true">→</i>}</div>)}
    </div></section>
    <section className="two-column"><div className="ledger-card"><div className="section-heading"><div><h2>ROMI по размещениям</h2></div><span className="section-note">на выручке</span></div><div className="table-wrap"><table><thead><tr><th>Канал</th><th>Затраты</th><th>Выручка</th><th>ROMI</th><th>Доказательство</th></tr></thead><tbody>{romi.length ? romi.map(row => <tr key={row.placementId}><td><b>{row.channel}</b><small>{row.placementId}</small></td><td>{rub.format(row.spend)}</td><td>{rub.format(row.attributedRevenue)}</td><td className={row.romi != null && row.romi >= 0 ? "positive" : ""}>{pct(row.romi)}</td><td><Confidence mix={row.confidenceMix} /></td></tr>) : <EmptyTableRow columns={5} text="Создайте placement, чтобы увидеть ROMI." />}</tbody></table></div></div>
      <div className="ledger-card"><div className="section-heading"><div><h2>Свежие оплаты</h2></div></div><ol className="event-list">{snapshot.orders.length ? snapshot.orders.slice(0,5).map(order => { const a = attributionByOrder.get(order.orderId); return <li key={order.orderId}><span className={`event-mark ${a?.placementId ? "verified" : "unknown"}`} /><div><b>{rub.format(order.amount)}</b><small>{order.course ?? "Курс не указан"} · {date(order.ts)}</small></div><span className={`pill ${a?.confidence ?? "unknown"}`}>{a?.confidence ?? "unknown"}</span></li>; }) : <li className="empty-state">Оплат пока нет</li>}</ol></div>
    </section>
  </div>;
}
...
function Registry({ snapshot, botUsername }: { snapshot: DashboardSnapshot; botUsername: string }) {
  ...
  return <div className="view-stack"><section className="ledger-card"><div className="section-heading"><div><h2>Кампании и deep links</h2></div><span className="section-note">{snapshot.placements.length} размещений</span></div><div className="table-wrap"><table><thead><tr><th>Кампания</th><th>Канал</th><th>Стоимость</th><th>Источник</th><th>Ссылка</th></tr></thead><tbody>{snapshot.placements.length ? snapshot.placements.map(p => { const link = botUsername ? buildDeepLink(botUsername, p.trackingToken) : null; return <tr key={p.placementId}><td><b>{campaignById.get(p.campaignId)?.name ?? p.campaignId}</b><small>{p.targetCourse ?? "Без курса"}</small></td><td>{p.channel}</td><td>{rub.format(p.cost)}</td><td><span className={`origin ${p.dataOrigin}`}>{originLabel[p.dataOrigin]}</span></td><td>{link ? <a className="deep-link" href={link} target="_blank" rel="noreferrer">Открыть</a> : <span className="muted">BOT_USERNAME не задан</span>}</td></tr>; }) : <EmptyTableRow columns={5} text="Сначала создайте campaign и placement." />}</tbody></table></div></section>
    <section className="ledger-card"><div className="section-heading"><div><h2>Касания и лиды</h2></div></div><div className="table-wrap"><table><thead><tr><th>Время</th><th>User key</th><th>Placement</th><th>Источник</th><th>Уверенность</th></tr></thead><tbody>{snapshot.touches.length ? snapshot.touches.map(t => <tr key={t.touchId}><td>{date(t.ts)}</td><td><code>{t.userKey}</code></td><td>{t.placementId ?? "organic"}</td><td>{t.source}</td><td><span className={`pill ${t.confidence}`}>{t.confidence}</span></td></tr>) : <EmptyTableRow columns={5} text="Касания появятся после перехода по tracking link." />}</tbody></table></div></section></div>;
}
...
function EmptyTableRow({ columns, text }: { columns: number; text: string }) { return <tr><td className="empty-table" colSpan={columns}>{text}</td></tr>; }
